...
void TrimPoisson::Alloc(int3 _tile_dim)
{
    ...
    is_dof_       = std::make_shared<DHMemory<uint8_t>>(voxel_num);
    ...
    tile_trimmed_ = std::make_shared<DHMemory<uint8_t>>(tile_num);
    cudaMemset((void*)(tile_trimmed_->dev_ptr_), 0x00, tile_num);
}

__global__ void TrimEmptyKernel(uint8_t* _tile_trimmed, int3 _tile_dim, const uint8_t* _is_dof)
{
...
__global__ void TrimTrivialKernel(uint8_t* _tile_trimmed, int3 _tile_dim, const uint8_t* _is_dof, const float* _a_diag, const float* _a_x, const float* _a_y, const float* _a_z, float _default_a_diag, float _default_a_off_diag)
{
    int tile_idx = blockIdx.x;
    int t_id     = threadIdx.x;
...
__global__ void LaplacianDotTrivialKernel(float* _result, int3 _tile_dim, float* _dot_buffer, const float* _x, const uint8_t* _tile_trimmed, float _default_a_diag, float _default_a_off_diag)
{
    ...
    uint8_t tile_trimmed = _tile_trimmed[tile_idx];
    if (tile_trimmed != 1)
        return;
    for (int i = 0; i < 4; i++) {
...
__global__ void LaplacianDotNonTrivialKernel(float* _result, int3 _tile_dim, float* _dot_buffer, const float* _x, const uint8_t* _tile_trimmed, const uint8_t* _is_dof, const float* _a_diag, const float* _a_x, const float* _a_y, const float* _a_z)
{
    int tile_idx  = blockIdx.x;
    int3 tile_ijk = TileIdxToIjk(_tile_dim, tile_idx);
    int t_id      = threadIdx.x;
...
__global__ void GaussSeidelRestrictTrivialKernel(float* _x, int3 _tile_dim, float* _coarse_b, const uint8_t* _tile_trimmed, float _default_a_diag, float _default_inv_a_diag, float _default_a_off_diag, const float* _b)
{
    __shared__ float shared_x[10][10][10];
...
__global__ void GaussSeidelRestrictNonTrivialKernel(float* _x, int3 _tile_dim, float* _coarse_b, const uint8_t* _tile_trimmed, const uint8_t* _is_dof, const float* _a_diag, const float* _a_x, const float* _a_y, const float* _a_z, const float* _b)
{
    __shared__ float shared_x[10][10][10];
    __shared__ float shared_b[8][8][8];
    __shared__ uint8_t shared_is_dof[10][10][10];
    int tile_idx  = blockIdx.x;
    int3 tile_ijk = TileIdxToIjk(_tile_dim, tile_idx);
    int t_id      = threadIdx.x;
...
__global__ void ProlongGaussSeidelDotTrivialKernel(float* _dst_x, int3 _tile_dim, const float* _x, const float* _coarse_x, const uint8_t* _tile_trimmed, float _default_inv_a_diag, float _default_a_off_diag, const float* _b, float* _dot_buffer, bool _do_dot)
{
    __shared__ float shared_x[10][10][10];
    __shared__ float shared_b[8][8][8];
    int tile_idx  = blockIdx.x;
...
__global__ void ProlongGaussSeidelDotNonTrivialKernel(float* _dst_x, int3 _tile_dim, const float* _x, const float* _coarse_x, const uint8_t* _tile_trimmed, const uint8_t* _is_dof, const float* _a_diag, const float* _a_x, const float* _a_y, const float* _a_z, const float* _b, float* _dot_buffer, bool _do_dot)
{
    __shared__ float shared_x[10][10][10];
    __shared__ float shared_b[8][8][8];
    __shared__ uint8_t shared_is_dof[10][10][10];
...
__global__ void CoarsestGaussSeidelKernel(float* _x, int3 _tile_dim, const uint8_t* _is_dof, const float* _a_diag, const float* _a_x, const float* _a_y, const float* _a_z, const float* _b, int _iter_num)
{
    int tile_idx  = blockIdx.x;
    int3 tile_ijk = TileIdxToIjk(_tile_dim, tile_idx);
    int t_id      = threadIdx.x;
